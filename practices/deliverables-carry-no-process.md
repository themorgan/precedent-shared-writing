...
**Three exemptions, and they are narrow.** The `<!-- Last updated ... -->` comment `file-header` requires is invisible in every rendering, so it stays. A `doc-recipes/` directory is exempt outright: a recipe's whole job is to state the rules for one file, and it is never shipped. And a section the repo lists under `unpublished_sections` in `precedent.json` is exempt, because that repo's own build cuts it before the document is sent. That list is the one place the cut is declared: whatever builds the sent page reads it too, so the check and the build can never disagree about what a reader sees.
